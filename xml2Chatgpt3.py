...
Converts the dictionary items (key-value pairs) into a list of tuples. Each tuple contains a key and its corresponding value.
columns=['Key', 'Value']:
Specifies the column names for the DataFrame. 'Key' will contain the keys from the dictionary, and 'Value' will contain the corresponding values.
'''
df_data = pd.DataFrame(list(data_dict.items()), columns=['Key', 'Value'])
...
# Create an ExcelWriter object
    with pd.ExcelWriter(filename, engine='openpyxl') as writer:
        # Convert the DataFrame to the first worksheet
        df.to_excel(writer, sheet_name=lot_id, index=False)

        # Convert the df_data DataFrame to the second worksheet
        df_data.to_excel(writer, sheet_name='AdditionalSheet', index=False)

    print("XML data has been converted to Excel successfully.")
else:
    print("Error: Both lot_id and qty must be provided.")