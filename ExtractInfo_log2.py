...
def write_to_file(output):
    with open(filename,"w",newline='') as f:
        writer = csv.DictWriter(f, fieldnames= ["SN", "Test site","Chip Temperature","killsystest",'read_dna','rpuburst_r001','apuburst_r001','aie2char_r001','contact_check','Versal_VC2802_ES1_LPNOM_ALL'],extrasaction='ignore')
        writer.writeheader()
        ...
        with open(os.path.join(cwd, file), 'r') as f:
            for line in f:
                ...
                if len(words)==2:
                    ...
                    if key == "Versal_VC2802_ES1_LPNOM_ALL":
                        print("Found!\n")
                        #val_time = re.sub(r'.*START (.*)::End(.*)::Result.*',r'\1\2',line)  #2nd arguements - subsituate with (1) or (2) by \1 or \1\2
                        val_time = re.sub(r'.*START (.*)::End(.*)::Result.*',r'\1',line)  #2nd arguements - subsituate with (1) or (2) by \1 or \1\2
