...
filename = "fileLogm1_4.csv"

def write_to_file(output):
    with open(filename,"w",newline='') as f:    #Modify the fieldnames of what is required to extract out from the log accordingly
        writer = csv.DictWriter(f, fieldnames= ["SN", "Test site","Chip Temperature",'DNA',
                                                "killsystest",'read_dna','rpuburst_r001','apuburst_r001','aie2char_r001',
                                                'contact_check','STARTDT','ENDDT'],extrasaction='ignore')
        writer.writeheader()
