...
class ChannelMonitor(Process):
    def __init__(self, channel_id, temp_queue, shared_state, lock, stop_event, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.channel_id = channel_id
        self.temp_queue = temp_queue
        self.shared_state = shared_state
        self.lock = lock
        self.stop_event = stop_event  # <-- NEW: shared stop signal
    ...
    def run(self):
        ...
        print(f"[Channel {self.channel_id}] Stopped gracefully.")
...
class TCUMasterProcess:
    def __init__(self, num_channels=4):
        ...
        self.temp_queue = self.manager.Queue()
        self.shared_state = self.manager.dict()
        self.lock = Lock()
        self.stop_event = Event()  # <-- NEW: Event to control stopping
        self.monitors = []
    ...
    def start_monitoring(self):
        ...
        for channel_id in range(1, self.num_channels + 1):
            monitor = ChannelMonitor(
                ...
                self.shared_state,
                self.lock,
                self.stop_event  # Pass the Event
            )
            monitor.start()
            self.monitors.append(monitor)

    def stop_monitoring(self):
        print("Stopping TCU Monitoring (gracefully)...")
        self.stop_event.set()  # Signal all processes to stop
        for monitor in self.monitors:
            monitor.join()  # Wait for each to finish
