...
df_tab2['FailCheck'] = df_tab2['Serial_Number'].map(
    lambda sn: 'TIC' if no_pass_serials.get(sn, False) else 'TestComplete'
)

# Step 1: Filter only rows with FailCheck == 'TIC'
tic_rows = df_tab2[df_tab2['FailCheck'] == 'TIC'].copy()
...
overall_df = pd.DataFrame(columns=['Serial_Number'] + test_eval_columns)
...
for sn, group in df_tab2.groupby('Serial_Number'):
    ...
    for col in test_eval_columns:
        # Get all values for this test case for the serial number
        values = group[col].dropna().astype(str).str.lower().str.strip()
        
        # Prioritize 'pass' if it exists, else 'fail' if it exists, else blank
        if 'pass' in values.values:
            overall_row[col] = 'pass'
        elif 'fail' in values.values:
            overall_row[col] = 'fail'
        else:
            overall_row[col] = ''  # leave blank if neither
...
with pd.ExcelWriter(output_excel, engine='xlsxwriter') as writer:
    ...
    tic_unique.to_excel(writer, sheet_name='TIC_Units', index=False)
    ...
    for sheet_name in ['Result_Summary', 'TIC_Units']:
